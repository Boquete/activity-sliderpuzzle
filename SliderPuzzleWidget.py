...
class SliderPuzzleMap (object):
    ...
    def do_move(self, slide_direction):
       ...
       """
        ...
        if self.hole_pos.move(slide_direction):
            ...
            self.pieces_map[
                old_hole_pos.y][
                old_hole_pos.x] = self.pieces_map[
                self.hole_pos.y][
                self.hole_pos.x]
            self.is_solved()
                ...
                if self.pieces_map[y][x] == piece:
                    if self.hole_pos.x == x:
                        if abs(self.hole_pos.y - y) == 1:
                            return self.do_move(
                                self.hole_pos.y > y and SLIDE_DOWN or SLIDE_UP)
                    elif self.hole_pos.y == y:
                        if abs(self.hole_pos.x - x) == 1:
                            return self.do_move(
                                self.hole_pos.x > x and SLIDE_RIGHT or SLIDE_LEFT)
                    else:
                        return False
        return False
    ...
    def get_cell_at(self, x, y):
        if x < 0 or x >= self.colsize or y < 0 or y >= self.rowsize or self.is_hole_at(
                x,
                y):
            return None
    ...
    def _freeze(self):
        return {
            ...
            'pieces_map': self.pieces_map,
            'hole_pos_freeze': self.hole_pos._freeze()}
...
class SliderPuzzleWidget (Gtk.Table):
    __gsignals__ = {
        'solved': (
            GObject.SIGNAL_RUN_LAST, GObject.TYPE_NONE, ()), 'shuffled': (
            GObject.SIGNAL_RUN_LAST, GObject.TYPE_NONE, ()), 'moved': (
                GObject.SIGNAL_RUN_LAST, GObject.TYPE_NONE, ()), }

    def __init__(self, pieces=9, width=480, height=480):
        self.jumbler = SliderPuzzleMap(pieces, self.jumblermap_piece_move_cb)
        # We take this from the jumbler object because it may have altered our
        # requested value
