...
class SliderPuzzleUI (Gtk.Table):
    ...
    def __init__(self, parent):
        ...
        # panel is a holder for everything on the left side down to (not
        # inclusive) the language dropdown
        panel = Gtk.VBox()
    ...
    def set_nr_pieces(self, btn, nr_pieces=None):
        ...
        #    return
        if self.is_contest_mode() and isinstance(
                btn, Gtk.ToggleButton) and nr_pieces == self.game.get_nr_pieces():
            return
        ...
        if btn is None:  # not isinstance(btn, Gtk.ToggleButton):
            if self._contest_mode:
                self.set_game_state(GAME_STARTED)
            for n, b in (
                    (9, self.btn_9), (12, self.btn_12), (16, self.btn_16)):
