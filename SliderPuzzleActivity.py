...
class GameTube (ExportedGObject):
    ...
    def need_image_cb(self, sender=None):
        ...
        # We will be sending the image, 24K at a time (my tests put the high
        # water at 48K)
        ...
        self.tube.get_object(
            sender,
            PATH).ImageDetailsSync(
            self.activity.frozen.freeze(),
            dbus_interface=IFACE)
    ...
    def ImageSync(self, image_part, part_nr):
        """ """
        logger.debug(
            "Received image part #%d, length %d" %
    ...
    def ImageDetailsSync(self, state):
        ...
        self.syncd_once = self.activity.frozen.thaw(
            str(state), forced_image=zlib.decompress(
                self.image.getvalue()), tube=self)
