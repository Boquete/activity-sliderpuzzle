...
# except for korea taken from
# http://zh.wikipedia.org/wiki/Image:Unification_flag_of_Korea.svg
    ...
    def guess_translation(self, fallback=False):
        try:
            self.gnutranslation = gettext.translation(
                self.domain, 'locale', languages=[
                    self.code], fallback=fallback)
            return True
        except:
            return False
...
def list_available_translations(domain):
    ...
    if not os.path.isdir('locale'):
        return rv
    for i, x in enumerate([x for x in os.listdir('locale') if os.path.isdir(
            'locale/' + x) and not x.startswith('.')]):
        try:
